...
namespace IngameScript
{
    partial class Program
    {
        public class Cpu
        {
            ...
            // Shortcut from the simemory
            public byte[] memory;
            public short ro_mem = 0x2000;
            ...
            private int[] cyclesValue = {
                ...
                5, 10, 10, 18, 11, 11, 7, 11,  5, 5,  10, 4 /*@@@ 5*/, 11, 17, 7, 11, //0xe0
                5, 10, 10, 4,  11, 11, 7, 11,  5, 5,  10, 4,           11, 17, 7, 11, //0xf0
                };

            public byte A, B, C, D, E, F, H, L;

            public ushort AF { get { return combineRegisters(A, F); } set { A = (byte)(value >> 8); F = (byte)(value & 0xD7 | 0x2); } }
            public ushort BC { get { return combineRegisters(B, C); } set { B = (byte)(value >> 8); C = (byte)value; } }
            public ushort DE { get { return combineRegisters(D, E); } set { D = (byte)(value >> 8); E = (byte)value; } }
            public ushort HL { get { return combineRegisters(H, L); } set { H = (byte)(value >> 8); L = (byte)value; } }

            public bool FlagS { get { return (F & 0x80) != 0; } set { F = value ? (byte)(F | 0x80) : (byte)(F & ~0x80); } }
            public bool FlagZ { get { return (F & 0x40) != 0; } set { F = value ? (byte)(F | 0x40) : (byte)(F & ~0x40); } }
            //private bool Flag0x20_0 { get { return (F & 0x20) != 0; } set { F = value ? (byte)(F | 0x20) : (byte)(F & ~0x20); } }
            public bool FlagAC { get { return (F & 0x10) != 0; } set { F = value ? (byte)(F | 0x10) : (byte)(F & ~0x10); } }
            //private bool Flag0x8_0 { get { return (F & 0x8) != 0; } set { F = value ? (byte)(F | 0x8) : (byte)(F & ~0x8); } }
            public bool FlagP { get { return (F & 0x4) != 0; } set { F = value ? (byte)(F | 0x4) : (byte)(F & ~0x4); } }
            public bool Flag0x2_1 { get { return (F & 0x2) != 0; } set { F = value ? (byte)(F | 0x2) : (byte)(F & ~0x2); } }
            public bool FlagC { get { return (F & 0x1) != 0; } set { F = value ? (byte)(F | 0x1) : (byte)(F & ~0x1); } }

            public byte M { get { return memory[HL]; } set { if (HL >= ro_mem) memory[HL] = value; } } // Vortex tries to clear ROM at start!
            public byte Data8 { get { return memory[PC]; } }
            public ushort Data16 { get { return BitConverter.ToUInt16(memory, PC); } }

            public Cpu(Memory memory, Bus iobus)
            {
                this.memory = memory.allProms[0];
                this.iobus = iobus;
                iobus.cpu = this;
            ...
            public void exe()
            {
                ...
                switch (opcode) {
                    ...
                    case 0xC0: RETURNCONDITIONAL(!FlagZ); break;          //RNZ	1		if NZ, RET
                    ...
                    case 0xC4: CALLCONDITIONAL(!FlagZ); break;            //CNZ adr	3		if NZ, CALLCONDITIONAL adr
                    case 0xC5: PUSH(BC); break;                //PUSH BC 	11 	1 	C5 Push 	(SP)=qq 
                    case 0xC6: ADD(Data8); PC += 1; break;     //ADI D8	2	Z, S, P, CY, AC	A <- A + byte
                    case 0xC7: RST(0x0); break;                //RST 0	1		CALLCONDITIONAL $0

                    case 0xC8: RETURNCONDITIONAL(FlagZ); break;           // RZ	1		if Z, RET
                    case 0xC9: RETURNALWAYS(); break;            // RET	1		PC.lo <- (sp); PC.hi<-(sp+1); SP <- SP+2
                    case 0xCA: JUMP(FlagZ); break;             // JZ adr 3       if Z, PC < -adr
                    case 0xCB: JUMP(true); break;              //JMP adr Alt Instruction C3 Clone
                    case 0xCC: CALLCONDITIONAL(FlagZ); break;             //CZ adr	3		if Z, CALLCONDITIONAL adr

                    case 0xCD:                                 //CALLCONDITIONAL adr	3		(SP-1)<-PC.hi;(SP-2)<-PC.lo;PC=adrç
                        CALLALWAYS();
                        break;

                    case 0xCE: ADC(Data8); PC += 1; ; break; //ACI D8	2	Z, S, P, CY, AC	A <- A + data + CY

                    case 0xCF: RST(0x8); break;         //RST 1	1		CALLCONDITIONAL $8

                    case 0xD0: RETURNCONDITIONAL(!FlagC); break;   //RNC	1		if NCY, RET
                    ...
                    case 0xD4: CALLCONDITIONAL(!FlagC); break;     //CNC adr	3		if NCY, CALLCONDITIONAL adr

                    case 0xD5: PUSH(DE); break;         //PUSH D	1		(sp-2)<-E; (sp-1)<-D; sp <- sp - 2
                    case 0xD6: SUB(Data8); PC += 1; ; break; //SUI D8	2	Z, S, P, CY, AC	A <- A - data

                    case 0xD7: RST(0x10); break;        //RST 2	1		CALLCONDITIONAL $10

                    case 0xD8: RETURNCONDITIONAL(FlagC); break;    // RC	1		if CY, RET
                    case 0xD9: RETURNALWAYS(); break;     // RET	1		PC.lo <- (sp); PC.hi<-(sp+1); SP <- SP+2 // Alt Instruction C9 Clone
                    case 0xDA: JUMP(FlagC); break;      // JC adr	3		if CY, PC<-adr

                    case 0xDB: A = iobus.Read(Data8); PC += 1; break; //IN A,(N) 	11 	2 	------ 	DB XX 	Input 	A=(n)

                    case 0xDC: CALLCONDITIONAL(FlagC); break;      //	CC adr	3		if CY, CALLCONDITIONAL adr
                    case 0xDD: CALLALWAYS(); break;       // Alt instruction CD Clone
                    case 0xDE: SBB(Data8); PC += 1; ; break; // SBI D8	2	Z, S, P, CY, AC	A <- A - data - CY

                    case 0xDF: RST(0x18); ; break;      // RST 3	1		CALLCONDITIONAL $18

                    case 0xE0: RETURNCONDITIONAL(!FlagP); break;   //	RPO	1		if PO, RET
                    ...
                    case 0xE3:                          //XTHL   1       L <-> (SP); H <-> (SP + 1)
                        ...
                        break;
                    case 0xE4: CALLCONDITIONAL(!FlagP); break;     //CPO adr	3		if PO, CALLCONDITIONAL adr
                    case 0xE5: PUSH(HL); break;         // PUSH H	1		(sp-2)<-L; (sp-1)<-H; sp <- sp - 2
                    case 0xE6: ANA(Data8); PC += 1; ; break; //ANI D8	2	Z, S, P, CY, AC	A <- A & data
                    case 0xE7: RST(0x20); break;        //RST 4	1		CALLCONDITIONAL $20
                    case 0xE8: RETURNCONDITIONAL(FlagP); break;    //RPE	1		if PE, RET
                    ...
                    case 0xEC: CALLCONDITIONAL(FlagP); break;      //CPE adr	3		if PE, CALLCONDITIONAL adr
                    case 0xED: CALLALWAYS(); break;       // Alt instruction CD Clone
                    case 0xEE: XRA(Data8); PC += 1; ; break; //XRI D8	2	Z, S, P, CY, AC	A <- A ^ data

                    case 0xEF: RST(0x28); break;        //RST 5	1		CALLCONDITIONAL $28
                    case 0xF0: RETURNCONDITIONAL(!FlagS); break;   //RP	1		if P, RET
                    ...
                    case 0xF4: CALLCONDITIONAL(!FlagS); break;     //CP adr	3		if P, PC <- adr
                    case 0xF5: PUSH(AF); break;         //PUSH AF 	11 	1 	------ 	F5 PUSH 	(SP)=qq
                    case 0xF6: ORA(Data8); PC += 1; ; break; //ORI D8	2	Z, S, P, CY, AC	A <- A | data
                    case 0xF7: RST(0x30); break;        //RST 6	1		CALLCONDITIONAL $30
                    case 0xF8: RETURNCONDITIONAL(FlagS); break;    //RM	1		if M, RET
                    ...
                    case 0xFC: CALLCONDITIONAL(FlagS); break;      // CM adr	3		if M, CALLCONDITIONAL adr
                    case 0xFD: CALLALWAYS(); break;       // Alt instruction CD Clone
                    case 0xFE: CMP(Data8); PC += 1; ; break; //CPI D8	2	Z, S, P, CY, AC	A - data

                    case 0xFF: RST(0x38); break; //RST 7	1		CALLCONDITIONAL $38
            ...
            private void RETURNALWAYS()
            {
                PC = POP();
            }
            private void RETURNCONDITIONAL(bool flag)
            {
                PC = flag ? POP() : PC;
                if (flag) cycles += 6;
            }

            private void CALLCONDITIONAL(bool flag)
            {
                if (flag) {
                    PUSH((ushort)(PC + 2));
                    PC = Data16;
